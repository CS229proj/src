...
def get_textcat_predictions(X):
    pred = []
    textcat = TextCat()
    print('Request started-TextCat')
    i = 0
    for text in X:
        try:
            i = i + 1
...
def main():
    ...
    pred = get_langid_predictions(Xgold)
    dump_results(Xgold, ygold, pred, 'langid.dat')
    cm = get_confusion_matrix(pred, ygold)
    save_cm(cm, 'output/benchmarks/langid_cm.png')

    #pred = get_textcat_predictions(Xgold)
    #dump_results(Xgold, ygold, pred, 'textcat.dat')
    #cm = get_confusion_matrix(pred, ygold)
    #save_cm(cm, 'output/benchmarks/textcat_cm.png')
